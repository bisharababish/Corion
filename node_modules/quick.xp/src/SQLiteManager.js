...
class SQLiteManager {
    ...
    */
    giveXP(message, xprate) {
        ...
        db.get(KEYS.REQUIRED_XP) || db.set(KEYS.REQUIRED_XP, 50);

        if (xp > required) {
            db.set(KEYS.PREVIOUSLY_REQUIRED, required);
            db.set(KEYS.LEVEL, nextlevel);

            if (!this.levelUpChannel && !this.dm) message.channel.send(this.substitute(message.author.tag, nextlevel)).then(m => {
                if (options.deleteMessage === true) m.delete({ timeout: 10000 })
            });
    ...
    */
    ...
    /**
     * getRank - Get Rank of the Specified User
     * @param {string} guildid 
     * @param {string} userid 
     */

    getRank(guildid, userid) {
        let all = this.db.all().filter(m => m.ID.startsWith(`xp_${guildid}`)).sort((a, b) => b.data - a.data);
        let rank = all.indexOf(all.find(m => m.ID === `xp_${guildid}_${userid}`)) + 1
        return rank;
    }

     /**
     * buildRankCard - Build Rank Card for the user
     * @param {object} message 
     * @param {string} background
     */

    async buildRankCard(message, userid, background) {
    const canvacord = require("canvacord");
    let user = message.client.users.cache.get(userid);
    let bg;
    if (background) bg = background;
    else if (!background) bg = "https://www.setaswall.com/wp-content/uploads/2019/08/Whatsapp-Wallpaper-102.jpg"
    const card = new canvacord.Rank()
    .setUsername(user.username)
    .setDiscriminator(user.discriminator)
    .setRank(this.getRank(message.guild.id, userid))
    .setLevel(this.getLevel(message, userid))
    .setCurrentXP(this.getXP(message, userid))
    .setRequiredXP(this.db.get(`requiredxp_${message.guild.id}_${userid}`))
